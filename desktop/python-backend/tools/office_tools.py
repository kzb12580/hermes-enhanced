...
# ═══════════════════════════════════════════════════════════════════════════
# PPT ANIMATION TOOLS — 基于 animate_ppt_engine
# ═══════════════════════════════════════════════════════════════════════════

def animate_ppt(path: str, animations: list, transitions: list = None, output: str = None) -> dict:
    """为PPT添加原生动画效果。
    
    Args:
        path: PPT文件路径
        animations: 动画列表 [{"slide": 1, "effect": "fade", "target": "all_text", ...}]
        transitions: 幻灯片切换效果 [{"slide": 1, "type": "fade", ...}]
        output: 输出路径
    """
    try:
        ...
        _log.error("animate_ppt failed: %s", e, exc_info=True)
        return {"error": str(e), "success": False}


def list_ppt_shapes(path: str, slide: int = None) -> dict:
    """列出PPT中的形状。
    ...
    """
    try:
        from .animate_ppt_engine import list_shapes
        result = list_shapes(path, slide_number=slide)
        if not result:
            return {"error": "No shapes found or file is empty", "success": False}
        # Format for LLM readability
        formatted = []
        for slide_num, shapes in sorted(result.items()):
            for name, sid in shapes.items():
                formatted.append(f"  Slide {slide_num}: [{sid}] {name}")
        return {"ok": True, "shapes": formatted, "total": sum(len(v) for v in result.values())}
    except Exception as e:
        _log.error("list_ppt_shapes failed: %s", e, exc_info=True)
        return {"error": str(e), "success": False}


def list_anim_effects() -> dict:
    """列出所有可用的动画效果。"""
    try:
        from .animate_ppt_engine import list_effects
        return {"ok": True, **list_effects()}
    except Exception as e:
        return {"error": str(e), "success": False}
