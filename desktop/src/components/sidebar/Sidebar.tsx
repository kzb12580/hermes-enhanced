...
export function Sidebar() {
  ...
  const { toggleSettings } = useSystemStore();
    ...
    <div className="flex flex-col h-full bg-[var(--bg-secondary)] border-r border-[var(--border)]">
      ...
      <div className="border-t border-[var(--border)] p-3">
        <button
          ...
          className="flex items-center gap-2 w-full px-3 py-2 rounded-lg text-[var(--text-muted)] hover:text-[var(--text-primary)] hover:bg-[var(--bg-tertiary)] transition-colors text-sm"
        >
          <Settings size={16} />
          <span>设置</span>
        </button>
