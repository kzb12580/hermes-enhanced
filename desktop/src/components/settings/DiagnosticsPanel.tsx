    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-medium text-[var(--text-primary)]">后端诊断</h3>
        </div>
        <div className="flex gap-2">
          <button onClick={runDiag} disabled={loading}
            className="flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs bg-[var(--bg-tertiary)] text-[var(--text-primary)] hover:bg-[var(--bg-secondary)] disabled:opacity-50 transition-colors">
            {loading ? <Loader2 size={12} className="animate-spin" /> : <RefreshCw size={12} />}
            刷新
          </button>
          <button onClick={reloadTools} disabled={reloading}
            className="flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs bg-[var(--accent)] text-white hover:opacity-90 disabled:opacity-50 transition-opacity">
      ...
      {message && (
        <div className={`p-3 rounded-lg text-xs ${message.startsWith('✅') ? 'bg-[var(--success)]/10 text-[var(--success)]' : 'bg-[var(--error)]/10 text-[var(--error)]'}`}>
          {message}
        </div>
      )}
        ...
        <div className="space-y-3">
          ...
          <div className="p-3 rounded-lg bg-[var(--bg-tertiary)] border border-[var(--border)]">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-xs font-medium text-[var(--text-primary)]">后端服务</span>
            </div>
          ...
          <div className="p-3 rounded-lg bg-[var(--bg-tertiary)] border border-[var(--border)]">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-xs font-medium text-[var(--text-primary)]">GPU</span>
            </div>
            {diag.gpu.available ? (
              <div className="grid grid-cols-2 gap-2 text-xs text-[var(--text-muted)]">
          ...
          <div className="p-3 rounded-lg bg-[var(--bg-tertiary)] border border-[var(--border)]">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-xs font-medium text-[var(--text-primary)]">视觉模型</span>
            </div>
            {diag.vision_model.found ? (
          ...
          <div className="p-3 rounded-lg bg-[var(--bg-tertiary)] border border-[var(--border)]">
            ...
            <div className="flex flex-wrap gap-1">
              {diag.tools.map(t => (
                <span key={t.name} className="px-2 py-0.5 text-xs bg-[var(--bg-primary)] text-[var(--text-secondary)] rounded">
                  {t.name}
                </span>
              ))}
            </div>
